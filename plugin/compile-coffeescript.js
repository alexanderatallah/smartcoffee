...
var compileFromCache = function(compileStep, future) {
  ...
  var srcMap = Storage.getItem('map-cache')[filename];
  if (src && src.length) {
...
var compile = function (compileStep, isLiterate, future) {
  ...
  cache[compileStep.inputPath] = sourceWithMap.source;
  Storage.setItem('cache', cache);

  var mapCache = Storage.getItem('map-cache');
  mapCache[compileStep.inputPath] = sourceWithMap.sourceMap;
  Storage.setItem('map-cache', mapCache);
...
var handler = function (compileStep, isLiterate) {
  ...
  fs.stat(compileStep.inputPath, function(err, stats) {
    ...
    if (doCompile) {
      ...
      compile(compileStep, isLiterate, future);
    } else {
